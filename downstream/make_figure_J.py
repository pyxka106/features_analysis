...
for color, subject in zip(colors, subjects):
    ...
    print(cd4_diversity_conv)
    x = cd4_diversity_conv.keys()
    print(x)

    cd4_ys = [cd4_diversity_conv[k] for k in x]

    x_1 = cd8_diversity[subject]
    cd8_diversity_conv = {float(k): v for k, v in x_1.items()}
    cd8_ys = [cd8_diversity_conv[k] for k in x]

    plot = plt.plot(x, cd4_ys, c=color, alpha=0.9, linewidth=2, label=subject)
    plt.plot(x, cd8_ys, c=color, linestyle='dashed', linewidth=2)
    plots.append(plot)
...
ax.set_ylim((0.03, 0.075))  # ((0.03, 0.075)) --j gene
